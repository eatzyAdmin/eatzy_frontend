...
/**
 * Hook to get wallet transactions for the logged-in restaurant
 */
export function useWalletTransactions(params?: {
    ...
    // Build filter for transaction types using restaurant-specific types
    // IN: RESTAURANT_EARNING
    // OUT: WITHDRAWAL, COMMISSION_PAID, ORDER_PAYMENT
    let typeFilter = "";
    if (params?.type === "IN") {
        typeFilter = "(transactionType~'RESTAURANT_EARNING')";
    } else if (params?.type === "OUT") {
        typeFilter = "(transactionType~'WITHDRAWAL' or transactionType~'COMMISSION_PAID' or transactionType~'ORDER_PAYMENT')";
    }

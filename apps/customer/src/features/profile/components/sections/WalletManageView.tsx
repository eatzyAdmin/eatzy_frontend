...
import { ChevronLeft, X, Info, MoreHorizontal } from "@repo/ui/icons";
...
export default function WalletManageView({ balance: propBalance, onBack, defaultType = 'TOPUP' }: WalletManageViewProps) {
  const { wallet, refresh } = useCustomerWalletTransactions();
  const balance = wallet?.balance ?? propBalance ?? 0;
  ...
  return (
    ...
    >
      <PullToRefresh
        ...
        pullText="Pull to update balance"
        releaseText="Release to update"
        refreshingText="Updating..."
      >
        <div className="min-h-full flex flex-col pb-0">
          {/* Header */}
          <div className="flex items-center justify-between px-2 h-14">
            <button onClick={onBack} className="flex items-center gap-1.5 group px-1">
              ...
              <span className="text-[17px] font-extrabold text-[#1A1A1A] tracking-tight">
                {activeType === 'TOPUP' ? 'Top-up' : 'Withdraw'}
              </span>
            </button>
          ...
          {/* Amount Display with Declarative Shake Logic */}
              ...
              {['1', '2', '3', '4', '5', '6', '7', '8', '9', '0', '000', 'DELETE'].map((key) => (
                ...
                >
                  {key === 'DELETE' ? (
                    ...
                    <span className={`text-[28px] font-extrabold text-gray-500 tracking-tight ${key === '000' ? 'text-[24px] mt-1' : ''}`}>
                      {key}
                    </span>
                  )}
                </motion.button>

...
import { Banknote } from "@repo/ui/icons";
      ...
      <div className="p-6 space-y-4 flex-1">
        ...
        <div className="flex justify-between items-center text-sm">
          ...
          <div className="text-right">
            {shippingDiscount > 0 && (
              <span className="line-through text-gray-400 text-xs mr-2">{formatVnd(fee + shippingDiscount)}</span>
            )}
            <span className="font-bold text-gray-900">{formatVnd(fee)}</span>
          </div>
        </div>
        ...
        {(discount > 0 || shippingDiscount > 0) && (
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-bold text-[var(--danger)]">-{formatVnd(discount + shippingDiscount)}</span>
          </div>
        )}
        ...
        <div className="flex justify-between items-center mt-auto pt-2">
          ...
          <div className="flex flex-col items-end">
            <span className="font-anton text-3xl text-[var(--primary)]">{formatVnd(Math.max(0, subtotal + fee - discount))}</span>

      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-0 md:gap-16">
          ...
          <div className="col-span-1 md:col-span-5 flex flex-col justify-center">
            <div className="text-[10px] text-amber-600 font-anton font-bold uppercase tracking-[0.4em] mb-4 md:mb-10 flex items-center">
              <span className="inline-block w-12 h-[2px] bg-amber-500 mr-4" />
              Signature Showcase
            </div>

            <h2 className="hidden md:block text-4xl md:text-9xl font-anton font-bold leading-[0.85] tracking-tighter mb-8 md:mb-10 text-black uppercase">
              LOOK<br />
              AT THIS<br />
            ...
            <div className="mb-4 md:mb-10 relative">
              ...
              <p className="text-gray-500 text-sm md:text-xl font-medium leading-relaxed max-w-sm">
                {restaurant.description}
              </p>
            </div>

            {/* Quote callout - Premium Glassmorphism */}
            <div className="hidden md:block relative p-6 md:p-8 rounded-[32px] bg-gray-50/50 backdrop-blur-xl border border-gray-100 overflow-hidden group/quote">
          ...
          <div onMouseEnter={(e) => moveHighlight(e, { borderRadius: 40, backgroundColor: 'rgba(0,0,0,0.06)', opacity: 1, scaleEnabled: true, scale: 1.02 })} className="col-span-1 md:col-span-7 relative z-10">
            <div className="relative aspect-[5/6] md:aspect-[3/4] overflow-hidden mb-8 rounded-[40px] shadow-2xl bg-gray-100 group">
              <ImageWithFallback
                src={hero.imageUrl}
                alt={hero.name}
                fill
              ...
              <div className="absolute bottom-8 left-4 right-4 md:left-8 md:right-8 p-5 md:p-8 bg-white/90 backdrop-blur-xl rounded-[32px] shadow-2xl border border-white/20 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-700">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <h3 className="text-xl md:text-4xl font-anton font-bold text-black uppercase tracking-tight">
                      {hero.name}
                    </h3>
                  </div>
                  <div className="text-right">
                    <span className="text-2xl md:text-4xl font-anton font-bold text-black leading-none">
                      {(hero.price / 1000).toFixed(0)}K
                    </span>
                  </div>
                </div>
                <p className="mt-2 md:mt-4 text-gray-500 text-xs md:text-sm font-medium leading-relaxed line-clamp-2">
                  {hero.description}
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 gap-y-4 md:gap-10 md:gap-y-10 mt-4 md:mt-16 pt-6 md:pt-16 border-t-[3px] border-black">
          {secondary.map((dish, idx) => (
            ...
            >
              <div className="relative aspect-square overflow-hidden mb-2 md:mb-6 rounded-[32px] shadow-lg border border-gray-100 bg-white">
                <ImageWithFallback
                  src={dish.imageUrl}
                  alt={dish.name}
                  fill
              ...
              <h4 className="font-anton font-bold text-lg md:text-2xl text-black uppercase tracking-tight mb-1 md:mb-2 px-2">{dish.name}</h4>
              <p className="md:block text-gray-500 text-xs md:text-sm font-medium md:leading-relaxed line-clamp-2 px-2">
                {dish.description}
              </p>
            </motion.div>
          ))}

      ...
      <div className="p-4 md:p-20 relative z-10">
        ...
        <div className="flex flex-col md:flex-row items-baseline justify-between gap-4 mb-6 md:mb-20 border-b border-white/10 pb-8">
          ...
          <div className="flex flex-col items-end">
            <div className="flex items-center gap-1.5 bg-white/10 backdrop-blur-md px-4 py-1.5 rounded-full border border-white/10">
              <Star className="w-4 h-4 fill-amber-400 text-amber-400" />
              <span className="font-anton text-xl font-bold translate-y-[2px]">{restaurant.rating}</span>
            </div>
          </div>
        </div>
        ...
        >
          ...
          {grid.map((dish, index) => (
            <motion.div
              key={dish.id}
              onMouseEnter={(e) => moveHighlight(e, { borderRadius: 64, backgroundColor: 'rgba(255,255,255,0.1)', opacity: 1, scaleEnabled: true, scale: 1.1, paddingX: 10, paddingY: 15 })}
            ...
            >
              ...
              <div className="relative aspect-[4/3] overflow-hidden rounded-tr-[50px] md:rounded-tr-[100px] rounded-bl-[50px] md:rounded-bl-[100px] mb-4 md:mb-8 bg-zinc-900 shadow-2xl border border-white/5">
                <ImageWithFallback
                  src={dish.imageUrl}
                  alt={dish.name}
                  fill
              ...
              <div className="flex-1 px-1 md:px-4">
                <div className="text-[8px] md:text-[10px] font-anton font-bold text-amber-500 uppercase tracking-widest mb-1 md:mb-2">Plate 0{index + 1}</div>
                <h3 className="text-base md:text-5xl font-anton font-bold mb-1 md:mb-4 uppercase tracking-tighter text-white group-hover:text-amber-500 transition-colors line-clamp-1 leading-tight py-1">
                  {dish.name}
                </h3>
                <p className="hidden md:block text-gray-400 text-sm md:text-lg font-medium leading-relaxed italic max-w-sm">
                  {dish.description}
                </p>
              </div>
            </motion.div>

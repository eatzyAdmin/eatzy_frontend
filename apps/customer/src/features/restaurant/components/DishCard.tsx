...
import { Plus, Minus, Info, Loader2 } from "@repo/ui/icons";
...
export default function DishCard({ dish, count, onAdd, onRemove, onClick, isLoading }: DishCardProps) {
  const {
    containerRef,
    rect,
              ...
              {count > 0 ? (
                <div className="flex items-center gap-1 md:gap-2 bg-black/40 backdrop-blur-md border border-white/10 rounded-full p-0.5 md:p-1 shadow-lg">
                  <button
                    ...
                    className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-all disabled:opacity-50"
                  >
                    {isLoading ? (

...
export default function TopUpDrawer({ open, onClose }: { open: boolean, onClose: () => void }) {
  ...
  const handleNext = () => {
    if (amountNumber < MIN_LIMIT) return;
    setStep('confirm');
  };
  ...
  const canContinue = amountNumber >= MIN_LIMIT;
                      ...
                      <div className="flex gap-2">
                        {[100000, 200000, 500000].map(v => (
                          <button
                            ...
                            className="flex-1 py-3 rounded-2xl bg-white border border-gray-100 text-sm font-bold text-gray-700 shadow-sm active:bg-gray-50"
                          >

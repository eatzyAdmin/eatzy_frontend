...
export function SearchPopup<T>({
  ...
  const handleInputChange = (key: keyof T, val: string) => {
    setLocalValues(prev => ({
      ...prev,
      [key]: val
    }));
  ...
  const handleClear = () => {
    const cleared = { ...localValues };
    fields.forEach(f => {
      (cleared as any)[f.key] = '';
    });
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="relative bg-white shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] rounded-[32px] p-6 flex flex-col gap-5 border border-gray-100/50">
            ...
            {/* Inputs Container */}
            <div className={`grid ${fields.length > 1 ? 'grid-cols-2' : 'grid-cols-1'} gap-4`}>
            ...
            <div className="flex justify-between items-center px-4 mt-1">
              <div className="flex gap-2">
                {fields.map((field) => (
                  <div key={String(field.key)} className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 uppercase tracking-widest bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-100">
                    {field.label}
                  </div>
                ))}
              </div>
